...
const msg3 = "wow it is so amazing";
...
const arr2 = ["quokka", "rabbit", "puppy", "hamster"];
arr1[5] = 8;
...
let arr5 = [1, 2, 3].map(function (el) {
    return el * 3;
});
arr5;
const arr6 = [1, 2, 3, 4, 5];
const arr7 = arr6.map((el) => el * 10);
arr7;
const arr8 = arr6.map((el) => {
    return String(el);
});
arr8;
let findResult = arr7.find((el) => {
    return el >= 30; //?
});
arr7;
findResult;
...
const 과일2 = ["수박", "사과", "참외", "오렌지", "파인애플", "망고"];
const same = [];
const diff = [];
...
// filter
//메서드 체이닝을 사용하기
const same2 = 과일1.filter((과일) => 과일2.includes(과일));
const diff2 = 과일1.filter((과일) => !과일2.includes(과일));
